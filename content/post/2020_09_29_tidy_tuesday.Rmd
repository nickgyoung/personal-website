...
---
![Beyonce: public domain license, Taylor Swift: Creative Commons, taken by David Shankbone](beyonce_swift.jpg)
...
beyonce_lyrics_total <- beyonce_lyrics %>% unnest_tokens(word, song_lyrics) %>% anti_join(stop_words, by = c('word' = 'word')) %>% filter(!word %in% custom_ts_stop_words, !word %in% custom_bey_stop_words) %>% group_by(word) %>% summarize(n = n()) %>% arrange(desc(n))
...
For next time, I'd like to get more practice with other viz packages, particularly ggarrange or other 